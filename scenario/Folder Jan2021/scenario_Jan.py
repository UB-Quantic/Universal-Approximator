...
########################################################;
    ...
    def add_lookup_tables(self, n_layers, theta_z):

        phases = np.zeros((len(self.x), n_layers), dtype=np.longdouble)

        for i_x in range(len(self.x)):
            accum_z_phase  = 0
            for n in range(n_layers):

                accum_z_phase += np.rad2deg( theta_z[i_x,n] )

                while accum_z_phase < 0:
                    accum_z_phase += 360
                while accum_z_phase >= 360:
                    accum_z_phase -= 360        
                phases[i_x,n] = accum_z_phase + 90

        
        for layer in range(n_layers):
            self._add_lookup_table(layer, [ x[layer] for x in phases] )
    ...
    def add_hadamard_gate(self, A_pi2):
        self._add_y_gate(-1, A_pi2)

        start_pulse_nr = 1 
        if self._meas_type == EXPERIMENT and \
            self._features["reset"]["isEnabled"]:
            start_pulse_nr += 2
        phase = [ feat("Phase #" + str(start_pulse_nr), 90) ]
        self._update_instrument( ALG, "Control Pulse", phase)        

    def add_y_gates(self, n_layers, A):
        for layer in range(n_layers):
            self._add_y_gate(layer, A[layer])
...
########################################################;
    ...
    def _prepare_exp_algorithm(self):
        ...
        self._prepare_exp_outputs(scn)
        self._add_reset(scn, self._features["reset"])
        self._generate_default_pulses(self._n_layers+1)
        self._add_measurement_pulse(scn)
    ...
    def _prepare_exp_outputs(self, scn):
        ...
        sqpg_pulse_feat =[
            ...
            feat("# of pulses", self._n_pulses +1)
        ]
    ...
    def _add_measurement_pulse(self, scn):
        ...
        sqpg_pulse_feat = [
            ...
            feat("Amplitude #"+str(meas_pulse), .105),
...
########################################################;
    ...
    def _add_lookup_table(self, layer, phase):
        start_pulse_nr = 2 # First there's a Hadamard gate
        if self._meas_type == EXPERIMENT and \
            self._features["reset"]["isEnabled"]:
            start_pulse_nr += 2

        lookup_step = self.alg_scn.add_step(
            channel = "Control Pulse - Phase #" + str(layer + start_pulse_nr),
            single = 0.0 )
        

        lookup = config.lookup.LookUpTable(self.x, phase)
    ...
    def _add_y_gate(self, layer, A):
        ...
        amplitude = [ feat("Amplitude #" + str(layer + start_pulse_nr), A) ]
        self._update_instrument( ALG, "Control Pulse", amplitude)
