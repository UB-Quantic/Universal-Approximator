...
# import labber.config.step as step
...
if __name__ == "__main__":
    ...
    scn.add_instrument(manual_info)
    ...
    # now add step
    scn._scn.add_step( "Manual - Value 1", x )
    # scn.add_step( instr_ch("Control Pulse", "Amplitude #1"), x)

    # now create lookup table
    lookup_step = scn._scn.add_step(
        channel = "Control Pulse - Amplitude #1",
        single = 0.0 )
    


    y = np.linspace(-np.pi*.1, np.pi*.1, 201)
    lookup = config.lookup.LookUpTable(x, y)

    # relation_param = step.RelationParameter() 
    # relation_param.variable = 'p1'
    # relation_param.channel_name =  'Manual - Value 1'
    param = config.step.RelationParameter()
    param.variable = 'p1'
    param.channel_name = 'Manual - Value 1'
    # param.channel_name = 'Control Pulse - Amplitude #1'
    ...
    # print(scn._scn.step_items[0])
    # print(scn._scn.step_items[1])
    # print(scn._scn.step_items[0].range_items)
    # print(scn._scn.step_items[1].range_items)
    # print(scn._scn.step_items[0].channel_name )
    # print(scn._scn.step_items[1].channel_name )


    scn.set_log_name('Test Universal Approximant')
    scn.save_log("test_univ_app")

    _RELPATH = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    measurement = ScriptTools.MeasurementObject(
                        os.path.join( _RELPATH, "test_univ_app.labber"),
